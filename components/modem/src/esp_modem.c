...
static void esp_handle_uart_data(esp_modem_dte_t *esp_dte)
{
    ...
    if (length)
    {
        ESP_LOG_BUFFER_HEXDUMP(MODEM_TAG, esp_dte->buffer, length, ESP_LOG_VERBOSE);
 ...
 *      ESP_FAIL on error
 */
static esp_err_t esp_modem_dte_send_wait(modem_dte_t *dte, const char *data, uint32_t length, const char *prompt, uint32_t timeout)
{
